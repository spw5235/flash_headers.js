...
if (isMAC && (navigator.userAgent.indexOf('Firefox/2') > 0)) {
    document.write('<style type="text/css">#nav_menus div, #ql_menu{ opacity:1 !important; -moz-opacity:1 !important; }</style>');
}

//remove transparency from menus in FF/Win to avoid issues with ClearType
if (isWin && (navigator.userAgent.indexOf('Firefox') > 0)) {
    document.write('<style type="text/css">#nav_menus div, #ql_menu{ opacity:1 !important; -moz-opacity:1 !important; }</style>');
}
    ...
    for (i = 0; i < fsLinks.length; i++) {
        if ( fsLinks[i].className.toLowerCase() == 'fsbtn' ) {
            ...
            if (fsLinks[i].childNodes[0].tagName) {
                ...
                fsLinks[i].onmouseover = function () {
                    fsBtn(this.childNodes[0].id);
                }
                fsLinks[i].onmouseout = function () {
                    fsBtn(this.childNodes[0].id, 'off');
                }
            ...
            if (use_overview_pages && document.getElementById(fsLinks[i].menu)) {
                fsLinks[i].href = document.getElementById(fsLinks[i].menu).getElementsByTagName('a')[0].href;
            }
        ...
        if ( fsLinks[i].className.toLowerCase() == 'qlbtn' ) {
            ...
            if (fsLinks[i].childNodes[0].tagName) {
                ...
                document.getElementById('ql_menu').onmouseover = function() {
                    clearTimeout(wait);
                }
                document.getElementById('ql_menu').onmouseout = function() {
                    mnuOut('ql_btn', 'ql_menu');
                }
    ...
    if (document.images) {
        ...
        for (this.btnName in FS.display.buttons) {
            if (FS.display.buttons[this.btnName].onSrc) {
                ...
                var n = 0;
                for (n in pagearray) {
                    // loop through pagearray to match pageid
...
function fsBtn(btn, state, alt_onSrc) {
    if (alt_onSrc) {
        if (alt_onSrc.indexOf(",") != - 1) {
            //check if there is a comma list with btn_name and src
            ob = alt_onSrc.split(",");
            xBtn = ob[0];
            xBtnImg = ob[1];
            swapImage(btn);
...
function swapImage(btn) {
    ...
    if (FS.display.buttons[btn].pageId != sectionId) {
        // btn src will not change if it is in the section, this preserves the selectSrc
    ...
    if (xBtn) {
        document.images[xBtn].src = xBtnImg;
    }
...
function imgRestore(btn) {
    ...
    if (FS.display.buttons[btn].menuName && document.getElementById(FS.display.buttons[btn].menuName)) {
        ...
        if (xBtn) {
            document.images[xBtn].src = FS.display.buttons[xBtn].offSrc;
        }
    }
}
...
function showSubs(btn) {
    if ((thisMenu.length > 0) && (thisMenu != FS.display.buttons[btn].btnName)) {
    ...
    if (section_subs == 1 || (FS.display.buttons[btn].pageId != sectionId)) {
...
function clearSubs(btn) {
    ...
    if (FS.display.buttons[btn].menuName) {
        if (anim_menus && mDir != "up") {
            ...
            $j('#' + FS.display.buttons[btn].menuName).slideUp(menu_speed * 1000, function() {
                btnOff(btn)
            });
        } else {
    ...
    if (document.getElementById(btn).className == "fsBtn_on" && FS.display.buttons[btn].pageId != sectionId) {
        document.getElementById(btn).className = "fsBtn";
    }
}
...
function fsMenu (btnName, menuName, menuPause) {
    ...
    function() {
        ...
        if (fsMenuName && menuName != fsMenuName) {
            $j(fsMenuName).hide();
        }
...
function setStyle() {
    ...
    if (style != null && document.getElementById('contentdiv')) {
        swapStyle(style);
    }
}
...
function fixContentHeight() {
    //use to fix the height of the content table in IE
...
function fsLinkRedirect () {
    ...
    if (fsRedirects.length > 0) {
        for (var i = 0; i < fsLinks.length; i++) {
            if (fsLinks[i].href.search('page.cfm') > - 1 && fsLinks[i].search.split(/p=/)[1]) {
                //limit to page.cfm links and check for p=
                id = fsLinks[i].search.split(/p=/)[1].split(/&/)[0]; // get the page id in each link
                for ( var j = 0; j < fsRedirects.length; j++) {
                    //loop through redirects list
...
function loadModalPanel () {
    ...
    if (isMAC && (navigator.userAgent.indexOf('Firefox/2') > 0)) {
        modalPanel.cfg.setProperty('underlay', 'none');
    }
...
$j(document).ready(function() {
    if (typeof YAHOO != "undefined") {
        YAHOO.util.Event.onDOMReady(loadModalPanel);
    }
});
...
function fsModalPanel(content, modalWidth, modalHeight, modalHeader, modalFooter) {
    ...
    {
        success: function(o) {
            showModalPanel(o.responseText, modalWidth, modalHeight, modalHeader, modalFooter) 
        },
        failure: function(o) {
            showModalPanel("The requested URL could not be loaded.", modalWidth, modalHeight, modalHeader, modalFooter) 
        }
    }
...
function showModalPanel (bodyText, modalWidth, modalHeight, modalHeader, modalFooter) {
    ...
    if (modalWidth) {
        modalPanel.cfg.setProperty('width', modalWidth);
    }
    //if(modalHeight){ modalPanel.cfg.setProperty('height', modalHeight); }

    if (modalHeader) {
        modalPanel.setHeader(modalHeader);
    }
    if (modalFooter) {
        modalPanel.setFooter(modalFooter);
    }
    ...
    document.getElementById('modalPanel_mask').onclick = function() {
        modalPanel.hide() 
    }
}
...
function gaTrackLinks() {
    ...
    if (typeof pageTracker != "undefined") {
        ...
        $j('a').filter( function() {
            return $j(this).attr('href') && $j(this).attr('href').indexOf('page.cfm') < 0 
        }).each(function() {

            var ah = $j(this).attr('href'); //grab the href attribute

            if ( ah.indexOf('#') == 0 || ah.indexOf('javascript:') == 0 ) {
                // filter out '#', anchors, and javascript links
                return; //do nothing			
            } else if (ah.indexOf('mailto') == 0) {
                //process mailto (mailMe links will be tracked through the mailMe function)
                $j(this).click(function() {
                    pageTracker._trackEvent('email links', ah , trackingpath + gaLinkText( $j(this) ));
                });
            } else if ( ah.indexOf('http') == 0 && ah.indexOf(document.location.host) < 0 ) {
                // process external links (not local domain)
                $j(this).click(function() {
                    pageTracker._trackEvent('external links', ah , trackingpath + gaLinkText( $j(this) ));
                });
            } else if (ah.indexOf('.cfm') > 0) {
                //filter local cfm here
                return;
            } else if (ah.indexOf('.htm') > 0) {
                // .htm files should get pageTracker function instead
                ...
                // assume the rest are file links
                $j(this).click(function() {
...
function writeMediaJS(divID, playername, data1, mediawidth, mediaheight, auto_tog, random_tog, loop_tog, delay_num, data7) {
    ...
    if (mediawidth == null) {
        ...
        if ((currWidth == 0) || (isNaN(currWidth))) {
            currWidth = null;
        }
    } else {
    ...
    if (mediaheight == null) {
        ...
        if ((currHeight == 0) || (isNaN(currHeight))) {
            currHeight = null;
        }
...
function embedFlash(divID, playername, embedWidth, embedHeight, aspectRatio, flashvars, params, attributes, minVersion) {
    $j(function() {
        ...
        if (typeof flashvars.mini == 'boolean' && !flashvars.mini) {
            if (typeof flashvars.siteurl == 'string') 
                expressInstallPath = flashvars.siteurl + '/cf_media/scripts/expressInstall.swf';
            else 
                expressInstallPath = '../cf_media/scripts/expressInstall.swf';
        }
        ...
        if (swfobject.hasFlashPlayerVersion(minVersion) || (swfobject.hasFlashPlayerVersion('6.0.65') && (typeof flashvars.mini == 'boolean' && !flashvars.mini))) 
            ...
            $j('#' + attributes.id + '_flash_requirement').show();
...
function fsUserLogin() {
    ...
    $j('#submitLogin').live('click', function() {
        ...
        $j.post(pathprefix + 'userlogin_remote.cfm', {
            doAction: "login",
            username: $j('#usernameLogin').val(),
            password: $j('#passwordLogin').val(),
            mediaPasskey: $j('#mediaPasskey').val(),
            mediaGroupId: postMediaGroupId,
            mediaCheckUser: postMediaCheckUser,
            mediaCheckKey: postMediaCheckKey 
        }, 
        function(data) {
        ...
        $j.ajax({
            ...
            success: function(data) {
                try {
                    ...
                    var result = {
                        'SUCCESS' : 0,
                        'WARNINGS': 'Update request failed during parse.' 
                    };
                }
...
function printpage() {
    ...
    if (thestring.length == 0) {
        thestring = 'p=' + pageid
    } else if (findid == - 1 && findp == - 1) {
        thestring = thestring + '&p=' + pageid
    }
    ...
    for (var f = 0; f < document.forms.length; f++)
    {
        for (var i = 0; i < document.forms[f].length; i++)
        {
            ...
            if ( document.forms[f].elements[i].tagName == "FIELDSET" || document.forms[f].elements[i].tagName == "fieldset" ) {
                i++ 
            }
...
function editorReturnImage(which_image) {
    if (which_image != '') {
        ...
        if (which_image.indexOf('file.cfm?') != - 1) {
            pathprefix = ''
        };
...
function editorUrlConverter(url, node, on_save) {
    ...
    //leave a tags alone
    if (node == 'a' || (typeof node == "object" && "nodeName" in node && node.nodeName === 'A')) 
        return url;
